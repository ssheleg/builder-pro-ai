...
mod tests {
    ...
    // ---- schema v2 graph tables, still present in the now-current schema v5 (fresh DB) ----

    #[test]
    fn fresh_db_is_schema_v5_with_graph_tables_and_all_five_indexes() {
        let db = new_db();
        let version: i64 = db
            .conn()
            .query_row("PRAGMA user_version", [], |r| r.get(0))
            .unwrap();
        // S-EXT spec §4 bumped SCHEMA_VERSION 2->3, S-IDEA spec §4 bumped it 3->4, and SCN-051
        // bumped it 4->5 (all additive); the S4 graph tables this test checks for are unaffected
        // — still created by `migrate_v2`, which `migrate_v3`/`migrate_v4`/`migrate_v5` build on
        // top of, never replace.
        assert_eq!(version, 5);
