...
mod tests {
    ...
    // ---- schema v2 graph tables, still present in the now-current schema v7 (fresh DB) ----

    #[test]
    fn fresh_db_is_schema_v7_with_graph_tables_and_all_five_indexes() {
        let db = new_db();
        let version: i64 = db
            .conn()
        ...
        // bumped it 4->5, SCN-054 bumped it 5->6, and SW1 bumped it 6->7 (all additive); the S4
        // graph tables this test checks for are unaffected — still created by `migrate_v2`, which
        // `migrate_v3`..`migrate_v7` build on top of, never replace.
        assert_eq!(version, 7);
