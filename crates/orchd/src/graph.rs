...
mod tests {
    ...
    // ---- schema v2 graph tables, still present in the now-current schema v8 (fresh DB) ----

    #[test]
    fn fresh_db_is_schema_v8_with_graph_tables_and_all_five_indexes() {
        let db = new_db();
        let version: i64 = db
            .conn()
            .query_row("PRAGMA user_version", [], |r| r.get(0))
            .unwrap();
        // S-EXT spec §4 bumped SCHEMA_VERSION 2->3, S-IDEA spec §4 bumped it 3->4, SCN-051
        // bumped it 4->5, SCN-054 bumped it 5->6, SW1 bumped it 6->7, and BL-120 bumped it 7->8
        // (all additive); the S4 graph tables this test checks for are unaffected — still
        // created by `migrate_v2`, which `migrate_v3`..`migrate_v8` build on top of, never
        // replace.
        assert_eq!(version, 8);
