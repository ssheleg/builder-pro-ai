...
| Per-session shell-integration assets | `session-<uuid>/` dirs next to the socket |
...
- **Dev (`npm run tauri dev`):** the core brings the daemon up itself using the dev build
  (`target/debug/bpa-sessiond`) — no LaunchAgent is installed for the dev daemon.
- **E2E harness:** spawns `target/debug/bpa-sessiond` directly with an **isolated
  `XDG_RUNTIME_DIR`** (tempdir), so it never touches your real daemon/socket
  (`tests/e2e/lib/daemon-harness.mjs`).
- Dev cleanup: kill the dev daemon (`pkill -f target/debug/bpa-sessiond`) and remove the isolated
  tempdir sockets if a run aborted.
