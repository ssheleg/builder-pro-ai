...
| **D4** | **Owner-driven fit-verdict; NO LLM.** Reuse `insight.fit_verdict`/`fit_reasoning`/`status`/`resolution_reasoning`. Fit-context (goals+metric_refs + graph neighborhood) is shown beside the insight; the owner decides. LLM auto-scoring → S6a. |
...
| **D6** | **Reuse verbs for everything but the run.** spawn-project (`CreateWorkspace`+`CreateProject`+`SetIdeaProject`), insight formation (`CreateInsight`+`SetInsightFitVerdict`+`SetInsightStatus`), task formation (`CreateTask`), decomposition (`CreateTask{parent_id}`) all reuse SHIPPED verbs. The ONLY net-new wire is the research verbs. |
...
CREATE TABLE research_run (
  ...
  invocation_id  TEXT,                          -- FK -> mcp_invocation(id); set when the call is recorded
  artifact_id    TEXT,                          -- FK -> mcp_artifact(id); set on 'done'
  error_kind     TEXT,                          -- set on 'failed' (e.g. 'policy_cap_exceeded','tool_error','transport'); NEVER a secret
...
3. `tokio::spawn` the run task (holds `Arc<Mutex<Db>>` + the broadcaster, NOT a `&Db` — 3-phase discipline):
   ...
   b. call `mcp::invoke::call_tool(db, server_id, tool_name, args_json, project_id = idea.project_id)` — this is the SHIPPED path: it authorizes (consent + spend/rate caps), invokes with retry/timeout, records `mcp_invocation` + (on success) the durable `mcp_artifact` (is_untrusted=1). NO DB lock held across its network await (it's already 3-phase).
   c. on `Ok(McpCallResult)`: lock→set `status:done, invocation_id, artifact_id`, push→unlock.
   d. on `Err`: lock→set `status:failed, error_kind` (map the typed error: `PolicyCapExceeded`→`policy_cap_exceeded`, tool/transport→their kinds), push→unlock. NEVER store args/secret/tool-output in `error_kind`.
...
**Graph-ingest (D9)** is NOT in the run path — it happens when the owner ACCEPTS the derived insight: the insight-accept UI calls the existing `SetInsightStatus{accepted}`; orchd's insight-accept handler additionally seeds an `entity_ref` node for the insight (a small addition to the shipped insight path, dedup by the partial-unique index). If touching the shipped insight handler is undesirable, an explicit `ResearchIngestInsight{insight_id}` verb is the fallback — but prefer folding it into accept.
...
- **orchd `research` unit** (in-memory Db + a fake `ToolCaller`/`connect_fn` seam like the S-EXT `mcp::invoke` tests): `start_run` inserts `pending` + flips idea `captured→researching`; the run task on a fake success sets `done`+artifact_id+invocation_id (artifact is the durable `mcp_artifact`, is_untrusted=1) + pushes; on a fake `PolicyCapExceeded` → `failed{policy_cap_exceeded}` + NO artifact + push; on a transport error → `failed{transport}`; the CHECK `(status='done')=(artifact_id NOT NULL)` holds; `error_kind` never contains args/secret. `list_runs`/`get_run`. Graph-ingest: accepting a research-insight seeds exactly one `entity_ref` node (dedup).
