...
/**
 * Shared «Оркестратор недоступен» banner (spec §10/§11 honest-degradation matrix: "orchd socket
 * down / connect refused ⇒ banner + retry; terminals unaffected"). A `statusExited` (red)
 * left-edge accent — NOT amber, which is reserved exclusively for "нужен ты"
 ...
 *
 * [Повторить] fires `orchdReconnect()` fire-and-forget: the call's own doc comment says its
...
export function OrchdDownBanner(): JSX.Element {
  return (
    <div role="alert" data-testid="orchd-down-banner" style={bannerStyle}>
      <span>Оркестратор недоступен</span>
      <button
        type="button"
